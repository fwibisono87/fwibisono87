<script lang="ts">
	...
	let isLoading = true;
	let eventList: EventMeta[] = [];

	onMount(async () => {
		try {
			const eventMeta = await fetch('/json/events.json');
			eventList = (await eventMeta.json()) as EventMeta[];
			isLoading = false;
		} catch (error) {
...
<div class="flex flex-col py-12 px-6">
	...
	<p class="text-center">
		...
		<a
			...
			target="_blank"
			aria-label="Send an email (opens in a new window)">photos@franciswibisono.com</a
		>
	</p>
	<p class="text-dove text-center mb-6">
		If you are a cosplayer who'd be interested in a photoshoot, feel free to email me too!
	</p>
	...
	<div class="flex flex-col gap-8" aria-live="polite">
		...
		{:else}
			{#each Array(12).fill(null) as _}
				<div
					class="relative bg-gray-600 animate-pulse rounded-lg border-text border-2 px-4 py-2 w-full md:w-[40%] mx-auto h-12"
				/>
			{/each}
		{/if}
	</div>
