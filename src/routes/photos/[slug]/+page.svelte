<script>
	...
	<div
		class="flex flex-col md:flex-row justify-between my-6 py-2 top-0 sticky bg-primary z-20 px-2"
	>
		...
		<a class="md:my-auto underline" target="_blank" href="/">@fwibisono87</a>
	</div>

	<!-- Gallery Grid -->
	<div class="grid grid-cols-3 lg:grid-cols-4 2xl:grid-cols-6 gap-4 w-[95%] mx-auto" role="list">
		{#if isLoading}
		{#each Array(12).fill() as _} <!-- Assume you want 12 skeletons -->
			<div class="relative bg-gray-600 animate-pulse aspect-square w-full h-full" transition:fade></div>
		{/each}
		{:else}
		{#each LR.images as image, index}
			<div
				...
				role="listitem"
				transition:fade={{ delay: 70*index }}
				tabindex="0"
				on:keydown={(e) => {
